        ...
        try:
            ...
            for line in r.text.split("\n"):
                ...
                try:
                    ...
                    if len(fields) < 4:
                        continue
                    # fields: 1=name, 2=code, 3=price, 4=prev_close, 5=open,
                    # 6=volume, 7=bid, 8=ask, ... 30=date, 31=time,
                    # 33=change_pct, 37=turnover, 43=industry(approx)
                    code = fields[2]
                    name = fields[1]
...
def _fetch_current_prices(codes: list[str]) -> dict[str, float]:
    ...
    # Tencent supports up to ~50 symbols per request
    symbols = []
    for code in codes:
        prefix = "sh" if code.startswith(("6",)) else "sz"
        symbols.append(f"{prefix}{code}")
                ...
                for line in r.text.split("\n"):
                    if '="' in line:
                        ...
                        if len(fields) > 3 and fields[3]:
                            # fields[2] is code, fields[3] is price
